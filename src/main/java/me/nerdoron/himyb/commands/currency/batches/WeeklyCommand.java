...
public class WeeklyCommand extends SlashCommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        int reward = Rng.generateNumber(25, 75);
        if (Global.COOLDOWN_MANAGER.hasCooldown(CooldownManager.commandID(event))) {
            ...
            event.reply("A week hasn't passed since you last claimed your weekly batch. " + remaining + " remaining.")
                    .setEphemeral(true)
                    .queue();
            return;
        }
    ...
    public SlashCommandData getSlash() {
        return Commands.slash("weekly", "Get a weekly batch of BroCoins.");
    }
}
