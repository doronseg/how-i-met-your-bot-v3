...
public class TransferCommand extends SlashCommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            logger.info("{}(ID:{}) transferred {} coins to {}(ID:{}).", member.getUser().getName(), member.getId(), transfer, memberToTransferTo.getUser().getName(), memberToTransferTo.getId());
    ...
    private MessageEmbed doneEmbed(Member member, Member memberToTransferTo, int transfer) {
        return new EmbedBuilder()
                ...
                .addField(memberToTransferTo.getUser().getName() + "'s Bank Balance", BROCOINS_SQL.getBroBank(memberToTransferTo) + " " + broCoin.getAsMention(), true)
